...
//import FieldElements.EnemyBot;
import robocode.*;
import FieldElements.*;
import FieldElements.Bullet;
...
public class DodgeyBoi extends AdvancedRobot {
    ...
    public static final double BOT_AVOIDANCE_CONSTANT = 1250; // used to be 500 but we had magic number 50 hanging out to determine power so.
    ...
    public void run() {
        ...
        while (true) {            
            ...
            for (int i = 0; i < bullets.size(); i++) {
                ...
                if (Math.hypot(getX() - b.x, getY() - b.y) < closestDistance) {
                    closestDistance = Math.hypot(getX() - b.x, getY() - b.y);
                    closestBullet = b;
                }
                if (Math.hypot(getX() - b.x, getY() - b.y) < Rules.getBulletSpeed(b.energy) + 16) {
                    // dodge algorithm
                    dodgeABullet(b);
                }
            }
    ...
    public void onScannedRobot(ScannedRobotEvent event) {
        updateBullets(event); // we have to do this first before updating the values of energy
    ...
    public void updateBullets(ScannedRobotEvent event) {
        double energy = 0;
        boolean newBullet = false;
        EnemyBot shooter = null;
        for (int i = 0; i < enemyBots.size(); i++) {
            if (event.getName().equals(enemyBots.get(i).name)) {
                energy = enemyBots.get(i).energy - event.getEnergy();
                newBullet = (energy > 0); // if the energy dropped between scans assume a bullet has been fired
                if (newBullet) shooter = enemyBots.get(i);
                break;
            }
        }
        if (newBullet) {
            // get the energy and stuff
            double x = shooter.x, y = shooter.y;
            double heading = Math.atan2(getX() - x, getY() - y);
            bullets.add(new Bullet(x, y, heading, energy, getTime()));
        }
        updateBullets();
    }

    public void updateBullets() {
        for (int i = 0; i < bullets.size(); i++) { // move the existing bullets
            bullets.get(i).updatePosition(getTime());
        }
    }
    ...
    */
    ...
    public void dodgeABullet(Bullet b) {
        // we want to move normal to the bullet's heading
        boolean goRight = false;
        double relativeHeading = Utils.normalAbsoluteAngle(b.heading - getHeadingRadians());
        if (relativeHeading < Math.PI) {
            goRight = true;
        }
        double absMoveDirection = Utils.normalRelativeAngle(getHeadingRadians() + (goRight ? Math.PI / 2.0 : - Math.PI / 2.0));
        double dist = 20;
        double turnAngle = Math.atan(Math.tan(absMoveDirection));
        setTurnRightRadians(turnAngle);
        if (turnAngle == absMoveDirection) {
            setAhead(dist);
        } else {
            setBack(dist);
        }
    ...
    public void fastAntiGravMove() {
        ...
        for (EnemyBot e: enemyBots) {
            ...
            xForce += Math.sin(absBearing) / Math.pow(Math.hypot(e.x - getX(), e.y - getY()), 2);
            yForce += Math.cos(absBearing) / Math.pow(Math.hypot(e.x - getX(), e.y - getY()), 2);
        }
        // wall repulsion
    ...
    public void antiGravMove() {
        ...
        for (EnemyBot e: enemyBots) {
            ...
            dx = getX() - e.x;
            dy = getY() - e.y;
            magn = e.energy * BOT_AVOIDANCE_CONSTANT / Math.pow(Math.hypot(e.x - getX(), e.y - getY()), 2); // always positive
    ...
    public boolean randomMove(boolean definite) {
        if (getClosestEnemyBot() != null) {
            if ((definite && Math.hypot(getClosestEnemyBot().x - getX(), getClosestEnemyBot().y - getY()) > CLOSE_QUARTERS_DISTANCE) || Math.random() <= CHANCE_OF_RANDOM_MOVE) {
                double xComp, yComp;
    ...
    public void predictiveShoot(EnemyBot e) {
        ...
        if (e != null) {         
            final double firePower = FIRING_DISTANCE / Math.hypot(e.x - getX(), e.y - getY());
    ...
    public EnemyBot getClosestEnemyBot() {
        ...
        for (EnemyBot e: enemyBots) {
            if (Math.hypot(e.x - getX(), e.y - getY()) < closestDistance) {
                closest = e;
                closestDistance = Math.hypot(e.x - getX(), e.y - getY());
            }
        }
        return closest;
    }
